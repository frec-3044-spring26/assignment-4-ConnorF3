...
ggplot(data = sunapee, aes(x = year, y = ice_off_doy))+
  geom_line()+
  geom_point()+
  labs(title = "Lake Sunapee Year vs Ice Off Day",
       x = "Year",
       y = "Ice Doy Off")
```

# How does ice cover change over time at Lake Sunapee?
The plot shows a warming trend with ice off dates being earlier the farther we go down the x axis. With the concentration of the points being in the bottom right hand corner, this means the ice is melting and leaving the lake earlier in the spring than it did in the late 1800s or early 1900s.
...
The direction of our slope is negative, so it does match my expectations from Question 2, with ice off day is nowhappening earlier.
...
The R2 for the regression based off year and ice of doy is .2055, so a fairly weak relation between the two variables.
...
ggplot(sunapee, aes(x = resid)) +
  ...
  labs(title = "Histogram of Residuals",
       ...
       y = "Count")
```
The residuals are approximately centered around zero, so it satisfies one of the key assumptions of linear regression. The distribution of the histogram is roughly bell shaped, maybe leaning towards a left skew shape due to larger negative residuals.  
...
The p-value for the slope is 2.5e-09, so very very small, providing strong evidence that slope is different from zero. So we reject that the slope is equal to 0.
...
The slope is -.10602, so ice off date is occurring .106 days earlier than the year prior. So 7 / .10602 = 66. So it would take 66 years to experience ice off date a week earlier. Yes, I think this is very meaning full it may seem like a long time for us in years, but with that much change in the climate in a small time frame seems meaning ful for climate implications.
...
I am moderately comfortable using this linear model for additional analysis. The slope is highly statistically significant p = 2.5e-09, and the residuals appear approximately normally distributed and centered around zero, satisfying key regression assumptions.

However, the R2 value is 0.2055, meaning that only about 20.5% of the variation in ice-off date is explained by year alone. The model is useful for identifying long-term trends but may have limited predictive power and should be used cautiously for forecasting.
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  ...
  geom_abline(intercept = 318.94856,
              ...
              color = "blue",
              size = 1) +
  labs(title = "Lake Sunapee Ice-Off Date Over Time",
       x = "Year",
...
quant <- quantile(sunapee$resid, probs = c(0.05, 0.95))


lower_90 <- pred_2040 + quant[1]
upper_90 <- pred_2040 + quant[2]
...
The predicted ice-off day in 2040 is approximately Day 102.7. Using the 5% and 95% quantiles of the residuals, the 90% confidence interval is approximately (87.7, 117.7). The confidence interval is about 33 days wide, indicating considerable uncertainty in the prediction.
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  ...
  geom_abline(intercept = coef(fit_early)[1],
              slope = coef(fit_early)[2],
              color = "blue",
              size = 1) +
  geom_abline(intercept = coef(fit_late)[1],
              slope = coef(fit_late)[2],
              color = "red",
              size = 1) +
  labs(title = "Ice-Off Date Before and After 1970",
       x = "Year",
       y = "Ice-Off Day of Year") +
  theme_minimal()
...
The slope before 1970 was approximately -0.10, while the slope after 1970 was -0.30. Because the slope in the later period is more negative, the ice-off day is decreasing more rapidly in recent years. This suggests that the rate of change has accelerated in the modern period.
